...
class AuditEngine:
    ...
    def _calc_axis(self):
        ...
        logic = np.mean([get_vec(w) for w in ["not", "no", "never", "false", "error", "nicht", "nein", "kein", "nie", "niemals", "falsch", "fehler"]], axis=0)
        struct = np.mean([get_vec(w) for w in ["however", "but", "and", "therefore", "jedoch", "daher", "aber", "und", "deshalb"]], axis=0)
        ax = logic - struct
    ...
    def scan_and_audit(self, pdf_text, page_map, query_text):
        ...
        # Gewichte berechnen (SPLADE Logik)
        token_weights = []
        for t in q_tokens_str:
            clean = t.replace(self.sp_token, "").strip()
            if not clean or not re.search(r'[a-zA-Z]', clean): w = 0.1
            elif clean.lower() in ["der", "die", "das", "und", "in", "von", "zu", "den", "dem"]: w = 0.2
            elif clean.lower() in ["bank", "kunde"]: w = 1.0 # Common Topic words
            else:
                w = min(6.0, len(clean) / 2.5) # Length heuristic
                if clean.lower() in ["nicht", "kein", "ohne", "minuten", "wochen"]: w = 8.0 
            token_weights.append(w)
        
        print(f"   -> Top Keywords: {[t for t,w in zip(q_tokens_str, token_weights) if w > 4.0]}")
    ...
    def _run_zipper(self, text_a, text_b):
        ...
        print(f"{'Typ':<10} | {'Wort':<25} | {'Score':<6}")
        print("-" * 40)
    ...
    def _check(self, word, is_ins, anomalies):
        ...
        if score > 0.10: 
            print(f"\033[91mLOGIC      | {word:<25} | {score:+.2f}\033[0m")
            anomalies.append(word)
        elif score > -0.05 and score <= 0.10:
             print(f"\033[93mCONTENT    | {word:<25} | {score:+.2f}\033[0m")
             anomalies.append(word)
        else:
             print(f"\033[95mOTHER      | {word:<25} | {score:+.2f}\033[0m")
             anomalies.append(word)
...
if __name__ == "__main__":
    ...
    engine.scan_and_audit(query_text, None, "Die Bank teilt dem Kunden Änderungen von Zinsen nicht mit.")
